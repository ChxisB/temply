import type { EditorThemeOptions, FallbackFont, FontProps } from './theme';
...
export function getVariableValue(key: string): string {
  ...
  return getComputedStyle(document.documentElement).getPropertyValue(key).trim() || DEFAULT_VALUES[key] ?? '';
}
...
export function getTemplyCssVariables(theme: EditorThemeOptions): Record<string, string> {
  ...
  if (font?.fallbackFontFamily) styles['--mly-fallback-font'] = font.fallbackFontFamily;
