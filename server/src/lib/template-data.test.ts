...
describe('collectDataKeys', () => {
  ...
  test('a document with neither yields empty lists', () => {
    expect(collectDataKeys({ type: 'doc', content: [{ type: 'paragraph' }] })).toEqual({
      conditions: [],
      variables: [],
    });
  });
  ...
  test('survives malformed input', () => {
    expect(collectDataKeys(null)).toEqual({ conditions: [], variables: [] });
    expect(collectDataKeys('nonsense')).toEqual({ conditions: [], variables: [] });
  });
});
